$(document).ready(function () {
    ...
    // Display a SweetAlert success message if "success" is defined
    {% if success %}
    swal({
        title: "Success!",
        text: "{{ success }}",
    ...
    // Add this script to handle folder deletion
    ...
    deleteFolderButtons.forEach((button) => {
        button.addEventListener("click", function (event) {
            ...
            swal({
                title: "Are you sure?",
                text: "Once deleted, you will not be able to recover this folder and its contents!",
                icon: "warning",
                buttons: true,
                dangerMode: true,
            }).then((willDelete) => {
                if (willDelete) {
                    ...
                    .catch((error) => {
                        console.error(error);
                        swal("Error", "Failed to delete the folder", "error");
                    });
                }
            });
        });
